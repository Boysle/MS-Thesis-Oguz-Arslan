...
from sklearn.metrics import f1_score, precision_score, recall_score, precision_recall_curve, confusion_matrix, accuracy_score, average_precision_score, log_loss
...
class RocketLeagueGCN(nn.Module):
    # This model is now flexible and matches your training scripts
...
class GraphLazyDataset(torch.utils.data.Dataset):
    # This Dataset is now flexible for 6-node or 7-node architectures
    def __init__(self, list_of_csv_paths, model_arch='6node'):
        ...
        print(f"\n--- Indexing complete. Total samples: {self.length}. Arch: {self.model_arch} ---")
    ...
    def __getitem__(self, idx):
        ...
        if not line: self.skipped_count += 1; return None
...
def get_predictions_and_loss(model, dataloader, device, pos_weight_orange, pos_weight_blue):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Getting predictions and loss"):
            ...
            # Calculate Loss
            total_loss_o += criterion_o(orange_logits, batch.y_orange.view_as(orange_logits)).item()
            total_loss_b += criterion_b(blue_logits, batch.y_blue.view_as(blue_logits)).item()
    ...
    avg_loss_o = total_loss_o / len(dataloader) if len(dataloader) > 0 else 0
    avg_loss_b = total_loss_b / len(dataloader) if len(dataloader) > 0 else 0
...
# ====================== MAIN EXECUTION ======================
def main():
    parser = argparse.ArgumentParser(description="Full Analysis for GNN Model")
    
    parser.add_argument('--model-path', type=str, required=True, help='Path to the saved GNN model checkpoint (.pth file).')
    parser.add_argument('--model-arch', type=str, required=True, choices=['6node', '7node'], help='Architecture of the GNN (6node or 7node).')
    ...
    output_dir_team = os.path.join(args.output_dir, f"{args.model_arch}_{args.team}")
    ...
    # --- 2. Load Data ---
    print(f"\n--- Loading VAL and TEST data (arch: {args.model_arch}) ---")
    ...
    val_dataset = GraphLazyDataset(val_files, model_arch=args.model_arch)
    test_dataset = GraphLazyDataset(test_files, model_arch=args.model_arch)
    ...
    try:
        ...
        print(f"Initializing GCN with: NodeFeat={node_features}, GlobalFeat={global_features}, HiddenDim={hidden_dim}")
        model = RocketLeagueGCN(node_features, global_features, hidden_dim).to(device)
    ...
    model_type_str = f'gnn_{args.model_arch}'
