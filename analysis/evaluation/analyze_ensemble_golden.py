...
# ====================== HELPER FUNCTIONS ======================
def load_and_normalize_data(list_of_csv_paths, exclude_kickoff):
    ...
    # --- Filter out kickoff rows if requested ---
    if exclude_kickoff and 'ball_hit_team_num' in full_df.columns:
        full_df = full_df[full_df['ball_hit_team_num'] != 0.5]
...
def load_raw_data_for_xgb(list_of_csv_paths, exclude_kickoff):
...
def plot_final_comparison_table(metrics_mlp, metrics_xgb, metrics_ensemble, team_name, output_dir):
    metrics_data = [[f"{metrics_mlp[0]:.4f}", f"{metrics_xgb[0]:.4f}", f"{metrics_ensemble[0]:.4f}"], [f"{metrics_mlp[1]:.4f}", f"{metrics_xgb[1]:.4f}", f"{metrics_ensemble[1]:.4f}"], [f"{metrics_mlp[2]:.4f}", f"{metrics_xgb[2]:.4f}", f"{metrics_ensemble[2]:.4f}"]]
    fig, ax = plt.subplots(figsize=(12, 2.5)); ax.axis('tight'); ax.axis('off'); table = ax.table(cellText=metrics_data, rowLabels=['Precision', 'Recall', 'F1-Score'], colLabels=['Optimized MLP', 'Optimized XGBoost', 'Averaged Ensemble (Th=0.5)'], cellLoc='center', loc='center'); table.auto_set_font_size(False); table.set_fontsize(12); table.scale(1.2, 1.2); plt.title(f'Final Model Comparison on Test Set ({team_name.upper()})', fontsize=16, pad=20); filepath = os.path.join(output_dir, f'final_model_comparison_{team_name}.png'); plt.savefig(filepath, bbox_inches='tight', dpi=150); plt.close(); print(f"  Final comparison table saved to: {filepath}")
...
def main():
    ...
    parser.add_argument('--exclude-kickoff', action='store_true', help='Whether to exclude kickoff events (ball_hit_team_num == 0.5).')
    ...
    golden_test_path = './goldset_test_wkickoff.csv'
    if not os.path.exists(golden_test_path):
        raise FileNotFoundError(f"Golden test dataset not found at {golden_test_path}")

    X_val_norm, y_orange_val, y_blue_val = load_and_normalize_data(val_files, False)
    X_val_raw, _, _ = load_raw_data_for_xgb(val_files, False)

    # Load the golden test as a single CSV (same feature expectations)
    X_test_norm, y_orange_test, y_blue_test = load_and_normalize_data([golden_test_path], args.exclude_kickoff)
    X_test_raw, _, _ = load_raw_data_for_xgb([golden_test_path], args.exclude_kickoff)
    ...
    print("\n--- Finding Optimal Thresholds for Baselines using VALIDATION set ---")
    ...
    print(f"  Optimal Thresholds (Orange): MLP={optimal_threshold_mlp_orange:.4f}, XGB={optimal_threshold_xgb_orange:.4f}")
    print(f"  Optimal Thresholds (Blue):   MLP={optimal_threshold_mlp_blue:.4f}, XGB={optimal_threshold_xgb_blue:.4f}")
    ...
    mlp_preds_opt_o = (mlp_probs_orange > optimal_threshold_mlp_orange).astype(int); xgb_preds_opt_o = (xgb_probs_orange > optimal_threshold_xgb_orange).astype(int)
    ensemble_probs_o = (mlp_probs_orange + xgb_probs_orange) / 2.0; ensemble_preds_o = (ensemble_probs_o > 0.5).astype(int)
    ...
    metrics_ensemble_o = [precision_score(y_orange_test, ensemble_preds_o), recall_score(y_orange_test, ensemble_preds_o), f1_score(y_orange_test, ensemble_preds_o)]
    plot_ensemble_distribution(y_orange_test, ensemble_probs_o, "orange", output_dir_o); plot_ensemble_confusion_matrix(y_orange_test, ensemble_preds_o, "orange", output_dir_o); plot_final_comparison_table(metrics_mlp_o, metrics_xgb_o, metrics_ensemble_o, "orange", output_dir_o)
    ...
    mlp_preds_opt_b = (mlp_probs_blue > optimal_threshold_mlp_blue).astype(int); xgb_preds_opt_b = (xgb_probs_blue > optimal_threshold_xgb_blue).astype(int)
    ensemble_probs_b = (mlp_probs_blue + xgb_probs_blue) / 2.0; ensemble_preds_b = (ensemble_probs_b > 0.5).astype(int)
    ...
    metrics_ensemble_b = [precision_score(y_blue_test, ensemble_preds_b), recall_score(y_blue_test, ensemble_preds_b), f1_score(y_blue_test, ensemble_preds_b)]
    plot_ensemble_distribution(y_blue_test, ensemble_probs_b, "blue", output_dir_b); plot_ensemble_confusion_matrix(y_blue_test, ensemble_preds_b, "blue", output_dir_b); plot_final_comparison_table(metrics_mlp_b, metrics_xgb_b, metrics_ensemble_b, "blue", output_dir_b)
