...
NUM_NODES_6 = 6; NODE_FEATURES_6 = 13; GLOBAL_FEATURES_6 = 14
NUM_NODES_7 = 7; NODE_FEATURES_7 = 12; GLOBAL_FEATURES_7 = 8
...
##### NEW/MODIFIED #####
# This class can now build EITHER model architecture
class RocketLeagueGAT(nn.Module):
    def __init__(self, node_features, global_features, hidden_dim, edge_dim, use_attention_pooling=False):
        super().__init__()
        ...
        if self.use_attention_pooling:
            # --- Layers for the 6-node (Attention Pool) model ---
            self.gate_nn = nn.Linear(hidden_dim, 1) 
            self.pool = GlobalAttention(gate_nn=self.gate_nn)
        
        # --- Common layers for both models ---
    ...
    def forward(self, data):
        ...
        # --- Choose the correct pooling method ---
        if self.use_attention_pooling:
...
# (GraphLazyDataset remains the same as the one I sent before)
...
# ====================== HELPER FUNCTIONS ======================
# (get_predictions_and_loss, plot_and_save_distribution, plot_and_save_confusion_matrix, 
#  plot_and_save_metrics_table, and calculate_class_weights are all identical to the 
#  GCN analysis script and are omitted here for brevity. Paste them in.)
# ...
...
def main():
    ...
    parser.add.argument('--model-arch', type=str, required=True, choices=['6node', '7node'], help='Architecture of the GAT (6node or 7node).')
    ...
    try:
        ...
        if args.model_arch == '6node':
            ...
            use_attention_pooling = True # 6-node model uses GlobalAttention
        else: # 7node
            node_features, global_features = NODE_FEATURES_7, GLOBAL_FEATURES_7
            use_attention_pooling = False # 7-node model uses global_mean_pool
            
        print(f"Initializing GAT with: NodeFeat={node_features}, GlobalFeat={global_features}, HiddenDim={hidden_dim}, EdgeDim={edge_dim}, AttentionPool={use_attention_pooling}")
        
        ##### NEW/MODIFIED #####
        # This is the key fix:
        model = RocketLeagueGAT(
            node_features, global_features, hidden_dim, edge_dim, 
    ...
    output_dir_team = os.path.join(args.output_dir, f"{model_type_str}_{args.team}")
    ...
    # --- 5. Run Analysis on VALIDATION Set ---
