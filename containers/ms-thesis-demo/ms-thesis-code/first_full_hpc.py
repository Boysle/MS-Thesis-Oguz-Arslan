...
def parse_args():
    ...
    parser.add_argument('--run-chrono-export', action='store_true', 
                        help="Run chronological analysis and export results after training. Disabled by default for HPC.")
    parser.add_argument('--chrono-output-filename', type=str, default="chronological_model_outputs.json",
                        help="Filename for the chronological analysis JSON output.")
...
def main():
    ...
    # ======== CHRONOLOGICAL ANALYSIS & EXPORT (Controlled by arg) ========
    if args.run_chrono_export: 
        output_json_path = os.path.join(os.path.dirname(args.checkpoint_path), args.chrono_output_filename)
        print(f"\n[CHRONO_ANALYSIS] Running chronological analysis and exporting to '{output_json_path}'...")
        ...
        # REMOVED the call to plot_state_from_chrono_item_in_script
        # The snippet printout from analyze_and_export_chronologically is sufficient for a quick log check.
        if all_chrono_results and args.debug: # Only print extra if debug is on
            print(f"[CHRONO_ANALYSIS] Debug: First item from chrono results: {all_chrono_results[0]}")
            if len(all_chrono_results) > 1:
                print(f"[CHRONO_ANALYSIS] Debug: Last item from chrono results: {all_chrono_results[-1]}")
    else:
        print("\n[CHRONO_ANALYSIS] Chronological analysis and export skipped (use --run-chrono-export to enable).")


    if wandb.run: 
        wandb.finish()
        print("[LOGGING] Weights & Biases run finished.")
    else:
        print("[LOGGING] No active Weights & Biases run to finish.")
