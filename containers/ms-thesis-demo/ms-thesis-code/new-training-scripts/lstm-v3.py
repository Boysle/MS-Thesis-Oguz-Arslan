...
class SequentialLazyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ##### NEW/MODIFIED #####
        # Get the ball_hit_team_num from the anchor row. Must use string.
...
class BaselineLSTM(nn.Module):
    ...
    def __init__(self, input_dim=TOTAL_FLAT_FEATURES, hidden_dim=128, num_layers=1, dropout=0.3):
        ...
        self.orange_head = nn.Sequential(
            ...
            nn.Dropout(p=dropout), nn.Linear(hidden_dim // 2, 1)
        )
...
# ====================== ARGUMENT PARSER ======================
def parse_args():
    # (This function is unchanged from v4)
    parser = argparse.ArgumentParser(description="Rocket League Baseline LSTM Training (v5 w/ OT Fix)")
...
def main():
    ...
    model = BaselineLSTM(input_dim=TOTAL_FLAT_FEATURES, hidden_dim=args.hidden_dim, num_layers=args.num_layers, dropout=args.dropout).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in pbar:
            ...
            epoch_overtime_outliers += overtime_outliers # Track new outlier
    ...
    if not os.path.exists(best_model_path):
        ...
        model = BaselineLSTM(input_dim=TOTAL_FLAT_FEATURES, hidden_dim=hidden_dim, num_layers=num_layers, dropout=dropout).to(device)
        model.load_state_dict(checkpoint['model_state'])
        ...
        ##### NEW/MODIFIED #####
        print(f"  Val Outliers Found: Replay={v_rep}, Score={v_sco}, Overtime={v_ot}")
        ...
        if not test_files:
            ...
            ##### NEW/MODIFIED #####
            print(f"  Test Outliers Found: Replay={t_rep}, Score={t_sco}, Overtime={t_ot}")
            ...
            try:
                ...
                wandb.summary["total_val_overtime_outliers"] = v_ot # <-- NEW
                wandb.summary["total_test_replay_outliers"] = t_rep
                wandb.summary["total_test_score_outliers"] = t_sco
                wandb.summary["total_test_overtime_outliers"] = t_ot # <-- NEW
