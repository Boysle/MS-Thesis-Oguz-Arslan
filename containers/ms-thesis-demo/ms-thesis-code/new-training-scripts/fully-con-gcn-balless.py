...
# ====================== ARGUMENT PARSER ======================
def parse_args():
    parser = argparse.ArgumentParser(description="Rocket League GCN Training Fully Connected Model without Ball Node") 
    ...
    parser.add_argument('--wandb-project', type=str, default="rl-gcn-test", help="W&B project name.")
    parser.add_argument('--run-name', type=str, default=None, help="Custom name for the W&B run.")
    parser.add_argument('--resume', action='store_true', help='Resume training from a checkpoint.')
    parser.add_argument('--checkpoint-path', type=str, default='./checkpoints_gcn/gcn_checkpoint.pth', help='Path for periodic checkpoint.')
    ...
    # parser.add_argument('--patience', type=int, default=10, help='Early stopping patience.') ##### NEW/FIXED ##### Removed per user request
...
# ##### NEW/FIXED #####
# This old evaluation function is no longer needed. 
# We will add a new evaluation block at the end of main() for rigor.
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if wandb.run:
            ...
            wandb.log({
                ...
                # Note: Plotting PR/ROC/CM every epoch can be very slow/heavy
                # "val/cm_orange": wandb.plot.confusion_matrix(...),
                # "val/pr_curve_orange": wandb.plot.pr_curve(...),
                # "val/roc_curve_orange": wandb.plot.roc_curve(...),
            })
        ...
        # We now check against avg_val_loss, not avg_val_f1
        if avg_val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'best_val_loss': best_val_loss, # Save the loss, not F1
                'args': vars(args), 
    ...
    # This entire block is new and ensures rigorous, standardized evaluation.
    ...
    if not os.path.exists(best_model_path):
        ...
        if not test_files:
            ...
            auprc_opt_o = average_precision_score(y_true_o, y_prob_o)
            auprc_opt_b = average_precision_score(y_true_b, y_prob_b)

            print("\n--- FINAL TEST RESULTS ---")
            print(f"  Default @ 0.5 (Orange): F1: {f1_def_o:.4f} | P: {prec_def_o:.4f} | R: {rec_def_o:.4f}")
            print(f"  Default @ 0.5 (Blue):   F1: {f1_def_b:.4f} | P: {prec_def_b:.4f} | R: {rec_def_b:.4f}")
            print(f"  Optimized @ {optimal_threshold_orange:.3f} (Orange): F1: {f1_opt_o:.4f} | P: {prec_opt_o:.4f} | R: {rec_opt_o:.4f}")
            print(f"  Optimized @ {optimal_threshold_blue:.3f} (Blue):   F1: {f1_opt_b:.4f} | P: {prec_opt_b:.4f} | R: {rec_opt_b:.4f}")
            print(f"  AUPRC (Orange): {auprc_opt_o:.4f}")
            print(f"  AUPRC (Blue):   {auprc_opt_b:.4f}")

            # --- Step 4: Log Final Summary to W&B ---
            # We re-init wandb to log this summary to the same run
            try:
                wandb.init(project=args.wandb_project, id=wandb_run_id, resume="allow")
                ...
                wandb.summary["optimized_test_auprc_orange"] = auprc_opt_o
                ...
                wandb.summary["optimized_test_auprc_blue"] = auprc_opt_b
