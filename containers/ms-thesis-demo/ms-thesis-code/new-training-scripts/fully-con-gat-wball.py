...
# ====================== ARGUMENT PARSER ======================
def parse_args():
    parser = argparse.ArgumentParser(description="Rocket League 7-Node GAT Training (Professional)")
...
class GraphLazyDataset(torch.utils.data.Dataset):
    def __init__(self, list_of_csv_paths, edge_feature_dim=4):
        ...
        print(f"\n--- Indexing complete. Total samples: {self.length}. Graph nodes: {NUM_NODES}. Edge features: {self.edge_feature_dim} ---")
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # This logic now works on the 7-node tensor
            positions = x_tensor[:, 0:3]  # Shape [7, 3]
            velocities = x_tensor[:, 3:6] # Shape [7, 3]
            forwards = x_tensor[:, 6:9]  # Shape [7, 3]
            teams = x_tensor[:, 10]       # Shape [7]

            edge_attrs = []
            # self.edge_index.t() is now 42 edges
            for i, j in self.edge_index.t():
                ...
                # Feature 2: Team Relationship (works for ball (id 2) vs player (id 0/1))
                ...
                # Feature 4: Forward Vector Alignment (ball fwd is [0,0,0], so dot will be 0)
...
def main():
    ...
    print(f"--- Using {args.edge_features} edge features for 7-node GAT ablation ---")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            ##### NEW/MODIFIED #####
            best_model_path = os.path.join(os.path.dirname(args.checkpoint_path), f'best_gat_7node_model_edge_{args.edge_features}.pth')
