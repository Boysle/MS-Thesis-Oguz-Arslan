...
class SequentialLazyDataset(Dataset):
    def __init__(self, list_of_csv_paths, sequence_length=6):
        ...
        # We start from seq_length-1 so we never ask for an index < 0
    ...
    def __len__(self):
        # We can't generate sequences for the first few rows
        return self.effective_length

    def _get_row(self, idx):
        """Helper function to get a single parsed row from the dataset."""
        if idx < 0 or idx >= self.length:
            return None, -1 # Invalid index, -1 file_index
        ...
        line = linecache.getline(file_path, local_idx + 2) # +2 for header and 1-based
        
        if not line.strip():
            return None, file_index # Empty line
            
        try:
            row = dict(zip(self.header, line.strip().split(',')))
            return row, file_index
        except (ValueError, KeyError, IndexError):
            return None, file_index # Corrupted line

    def _normalize_and_flatten(self, row):
        """Builds the 92-feature normalized tensor from a data row."""
        # This function is slow, but necessary for a non-DataFrame loader
        features = []
        try:
            for col in self.feature_cols_ordered:
                ...
                else: features.append(val) # For team, alive, ball_hit_team_num
    ...
    def __getitem__(self, idx):
        # We are fetching a sequence for the data point at 'idx'
        # But 'idx' in the dataloader is from 0 to self.effective_length
        # So, the "current" data point's real index is:
        current_idx = idx + self.start_index

        sequence_rows = []
        outlier_count_replay = 0
        outlier_count_score = 0
        
        # 1. Get the anchor row (the most recent data point, 't')
        anchor_row, anchor_file_idx = self._get_row(current_idx)
        if anchor_row is None:
            return None # This row is bad, skip
        ...
        # 2. Iterate backward for the 5 previous frames
        for k in range(1, self.sequence_length):
            ...
            # Check 1: Row is bad, or file boundary
            if prev_row is None or prev_file_idx != anchor_file_idx:
                is_valid = False
                outlier_count_replay += 1
            # Check 2: Replay boundary
            elif prev_row['replay_id'] != anchor_replay_id:
                is_valid = False
                outlier_count_replay += 1
            # Check 3: Score (kickoff) boundary
            elif prev_row['score_difference'] != anchor_score_diff:
                is_valid = False
                outlier_count_score += 1
            
            # Add to sequence
            if is_valid:
                sequence_rows.insert(0, prev_row) # Add to the beginning
                last_valid_row = prev_row
            else:
                # Outlier found: pad by replicating the last valid row
                sequence_rows.insert(0, last_valid_row)

        # 3. Build feature tensors and labels
        x_seq_tensors = []
        for row in sequence_rows:
            features = self._normalize_and_flatten(row)
            if features is None:
                return None # Corrupted data in one of the rows
            x_seq_tensors.append(features)
        
        x_tensor = torch.stack(x_seq_tensors) # Shape [6, 92]
...
class BaselineLSTM(nn.Module):
    def __init__(self, input_dim=TOTAL_FLAT_FEATURES, hidden_dim=128, num_layers=1):
        super(BaselineLSTM, self).__init__()
        self.lstm = nn.LSTM(
            input_size=input_dim,
            hidden_size=hidden_dim,
            num_layers=num_layers,
            batch_first=True  # This is crucial: [batch_size, seq_len, features]
        )
        
        # Two separate heads, as requested
        self.orange_head = nn.Sequential(nn.Linear(hidden_dim, hidden_dim // 2), nn.ReLU(), nn.Linear(hidden_dim // 2, 1))
        self.blue_head = nn.Sequential(nn.Linear(hidden_dim, hidden_dim // 2), nn.ReLU(), nn.Linear(hidden_dim // 2, 1))
...
def calculate_class_weights(train_files):
    """Scans training files to calculate pos_weight for loss functions."""
...
def get_predictions_and_loss_sequential(model, loader, device, criterion_o, criterion_b):
    """Adapted function for sequential data and outlier tracking."""
    model.eval()
...
# ====================== ARGUMENT PARSER ======================
def parse_args():
    parser = argparse.ArgumentParser(description="Rocket League Baseline LSTM Training")
    ...
    parser.add_argument('--wandb-project', type=str, default="rl-lstm-test", help="W&B project name.")
...
def main():
    ...
    model = BaselineLSTM(input_dim=TOTAL_FLAT_FEATURES, hidden_dim=args.hidden_dim, num_layers=args.num_layers).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        val_replay_outliers, val_score_outliers = 0, 0
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc=f"Epoch {epoch+1}/{args.epochs} [VAL]"):
                ...
                val_replay_outliers += replay_outliers
                val_score_outliers += score_outliers
                
                orange_logits, blue_logits = model(x_batch)
                total_val_loss += (criterion_orange(orange_logits, y_o_batch) + criterion_blue(blue_logits, y_b_batch)).item()
                
                all_val_oprobs.extend(torch.sigmoid(orange_logits).cpu().numpy().flatten())
                all_val_olabels.extend(y_o_batch.cpu().numpy().flatten())
                all_val_bprobs.extend(torch.sigmoid(blue_logits).cpu().numpy().flatten())
                all_val_blabels.extend(y_b_batch.cpu().numpy().flatten())
        
        avg_val_loss = total_val_loss / len(val_loader) if len(val_loader) > 0 else 0
        
        # Calculate validation metrics
        np_val_olabels = np.array(all_val_olabels); np_val_oprobs = np.array(all_val_oprobs)
        np_val_blabels = np.array(all_val_blabels); np_val_bprobs = np.array(all_val_bprobs)
    ...
    if not os.path.exists(best_model_path):
        ...
        # Re-init criteria for final eval
        ...
        val_labels_o, val_labels_b, val_probs_o, val_probs_b, val_loss_o, val_loss_b, v_rep, v_sco = get_predictions_and_loss_sequential(
        ...
        if not test_files:
            ...
            y_true_o, y_true_b, y_prob_o, y_prob_b, test_loss_o, test_loss_b, t_rep, t_sco = get_predictions_and_loss_sequential(
            ...
            # ... (Full metric calculation block as in previous scripts) ...
            ...
            try:
                ...
                # ... (Logging all default and optimized metrics) ...
                # ... (This section is identical to your other scripts, omitted for brevity) ...
                
                # Log final outlier counts
                wandb.summary["total_train_replay_outliers"] = epoch_replay_outliers
                wandb.summary["total_train_score_outliers"] = epoch_score_outliers
