...
class XGBoostDataLoader:
    ...
    def to_dataframe(self):
        ...
        ##### THIS IS THE FIX #####
        # Define the EXACT feature order from the analysis script
        player_cols = [
        ...
        return X, y_orange, y_blue, feature_cols
...
def main():
    ...
    # Load all data using the new, standardized function
    # We set normalize_for_mlp=False because XGBoost does not need it.
    
    print("\n--- Loading Training Data ---")
    X_train, y_orange_train, y_blue_train = load_data_as_df(train_files, normalize_for_mlp=False)
    
    print("\n--- Loading Validation Data ---")
    X_val, y_orange_val, y_blue_val = load_data_as_df(val_files, normalize_for_mlp=False)
    
    print("\n--- Loading Test Data ---")
    X_test, y_orange_test, y_blue_test = load_data_as_df(test_files, normalize_for_mlp=False)

    if X_train is None:
        print("CRITICAL ERROR: No training data could be loaded. Exiting.")
        return

    # Get the feature names from the DataFrame columns, which are now in the correct order
