...
from torch_geometric.nn import GATConv, global_mean_pool, GlobalAttention # <-- Added GlobalAttention
import wandb
...
# ====================== ARGUMENT PARSER ======================
def parse_args():
    parser = argparse.ArgumentParser(description="Rocket League 7-Node GAT Training (w/ Attention Pooling)")
    ...
    parser.add_argument('--wandb-project', type=str, default="rl-gat-7node", help="W&B project name.")
    parser.add_argument('--run-name', type=str, default=None, help="Custom name for the W&B run.")
    parser.add_argument('--resume', action='store_true', help='Resume training from a checkpoint.')
    parser.add_argument('--checkpoint-path', type=str, default='./checkpoints_gnn/gnn_7node_attn_checkpoint.pth', help='Path for periodic checkpoint.')
...
class GraphLazyDataset(torch.utils.data.Dataset):
    def __init__(self, list_of_csv_paths, edge_feature_dim=4):
        ...
        # Set constants for 7-node architecture
        ...
        print(f"\n--- Indexing complete. Total samples: {self.length}. Graph nodes: {self.num_nodes}. Edge features: {self.edge_feature_dim} ---")
...
# ====================== MODEL ARCHITECTURE ======================
##### NEW/MODIFIED #####
# This is the GAT class with GlobalAttention Pooling
class RocketLeagueGAT(nn.Module):
    ...
    def forward(self, data):
        ...
        # 4. Use the new attention pooling layer
...
def main():
    ...
    print(f"--- Using {args.edge_features} edge features for 7-node GAT (Attn Pool) ablation ---")
    ...
    # Initialize model with 7-node constants
    model = RocketLeagueGAT(NODE_FEATURES, GLOBAL_FEATURES, args.hidden_dim, edge_dim=args.edge_features).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(os.path.dirname(args.checkpoint_path), f'best_gat_7node_attn_model_edge_{args.edge_features}.pth') # New name
    ...
    # (Data Loading Summary and W&B Finish)
    ...
    if not os.path.exists(best_model_path):
        ...
        # (Full evaluation block as in other scripts...)
        ...
        if not test_files:
            ...
            with torch.no_grad():
                for batch in tqdm(test_loader, desc="[FINAL TEST]"):
                    ...
                    batch = batch.to(device)
                    orange_logits, blue_logits = model(batch)
                    all_test_oprobs.extend(torch.sigmoid(orange_logits).cpu().numpy().flatten()); all_test_olabels.extend(batch.y_orange.cpu().numpy().flatten())
                    all_test_bprobs.extend(torch.sigmoid(blue_logits).cpu().numpy().flatten()); all_test_blabels.extend(batch.y_blue.cpu().numpy().flatten())

            # (Full metric calculation, printing, and W&B logging as in other scripts...)
            # ...
