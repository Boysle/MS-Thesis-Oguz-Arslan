...
# The model's architecture must be defined to load the weights correctly.
...
# A simplified dataset for evaluation that loads a DataFrame into memory.
class EvaluationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.features[idx], self.orange_labels[idx], self.blue_labels[idx]

def load_and_prepare_data(list_of_csv_paths):
    """Loads all chunks, processes them, and returns NumPy arrays."""
    ...
    for _, row in tqdm(full_df.iterrows(), total=len(full_df), desc="Processing rows"):
        # This logic is identical to your training script's data processing
...
def get_predictions(model, dataloader, device):
    """Runs inference and returns true labels and predicted probabilities."""
    model.eval()
...
def main():
    ...
    parser.add_argument('--model-path', type=str, required=True, help='Path to the saved MLP model (.pth file).')
    ...
    _, _, val_probs_orange, val_probs_blue = get_predictions(model, val_loader, device)
    ...
    _, _, test_probs_orange, test_probs_blue = get_predictions(model, test_loader, device)
    ...
    f1_def_o = f1_score(y_orange_test, test_preds_default_orange)
    prec_def_o = precision_score(y_orange_test, test_preds_default_orange)
    rec_def_o = recall_score(y_orange_test, test_preds_default_orange)
    ...
    f1_opt_o = f1_score(y_orange_test, test_preds_optimized_orange)
    prec_opt_o = precision_score(y_orange_test, test_preds_optimized_orange)
    rec_opt_o = recall_score(y_orange_test, test_preds_optimized_orange)
    print(f"  Optimized Orange -> F1: {f1_opt_o:.4f} | Precision: {prec_opt_o:.4f} | Recall: {rec_opt_o:.4f}")
    
    f1_opt_b = f1_score(y_blue_test, test_preds_optimized_blue)
    prec_opt_b = precision_score(y_blue_test, test_preds_optimized_blue)
    rec_opt_b = recall_score(y_blue_test, test_preds_optimized_blue)
