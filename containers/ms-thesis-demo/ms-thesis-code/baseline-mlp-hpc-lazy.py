...
def main():
    ...
    if args.resume:
        if os.path.exists(args.checkpoint_path):
            ...
            print(f"--- WARNING: --resume flag was set, but no checkpoint found at '{args.checkpoint_path}'. Starting from scratch. ---")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        model.eval()
        total_val_loss, val_probs_o, val_labels_o, val_probs_b, val_labels_b, val_preds_o_binary, val_preds_b_binary = 0, [], [], [], [], [], []
        with torch.no_grad():
            for features, orange_labels, blue_labels in tqdm(val_loader, desc=f"Epoch {epoch+1}/{args.epochs} [VAL]"):
                features, orange_labels, blue_labels = features.to(device), orange_labels.to(device), blue_labels.to(device)
                ...
                val_probs_o.extend(orange_pred_prob.cpu().numpy().flatten()); val_labels_o.extend(orange_labels.cpu().numpy().flatten())
                val_probs_b.extend(blue_pred_prob.cpu().numpy().flatten()); val_labels_b.extend(blue_labels.cpu().numpy().flatten())
                val_preds_o_binary.extend((orange_pred_prob.cpu().numpy() > 0.5).astype(int).flatten()); val_preds_b_binary.extend((blue_pred_prob.cpu().numpy() > 0.5).astype(int).flatten())

        avg_train_loss = total_train_loss / len(train_loader); avg_val_loss = total_val_loss / len(val_loader)
        val_f1_o = f1_score(val_labels_o, val_preds_o_binary, zero_division=0); val_f1_b = f1_score(val_labels_b, val_preds_b_binary, zero_division=0)
        val_prec_o = precision_score(val_labels_o, val_preds_o_binary, zero_division=0); val_recall_o = recall_score(val_labels_o, val_preds_o_binary, zero_division=0)
        val_prec_b = precision_score(val_labels_b, val_preds_b_binary, zero_division=0); val_recall_b = recall_score(val_labels_b, val_preds_b_binary, zero_division=0)
    ...
    if not os.path.exists(best_model_path):
        ...
        if not test_files:
            ...
            with torch.no_grad():
                for features, orange_labels, blue_labels in tqdm(test_loader, desc="[FINAL TEST]"):
                    features, orange_labels, blue_labels = features.to(device), orange_labels.to(device), blue_labels.to(device)
                    orange_pred, blue_pred = model(features)
            ...
            test_f1_o = f1_score(test_labels_o, test_preds_o_binary, zero_division=0); test_f1_b = f1_score(test_labels_b, test_preds_b_binary, zero_division=0)
            test_prec_o = precision_score(test_labels_o, test_preds_o_binary, zero_division=0); test_recall_o = recall_score(test_labels_o, test_preds_o_binary, zero_division=0)
            test_prec_b = precision_score(test_labels_b, test_preds_b_binary, zero_division=0); test_recall_b = recall_score(test_labels_b, test_preds_b_binary, zero_division=0)
            ...
            if wandb.run:
                wandb.summary["best_epoch"] = checkpoint['epoch'] + 1
                wandb.summary["test_f1_orange"] = test_f1_o; wandb.summary["test_f1_blue"] = test_f1_b
