...
def main():
    ...
    if args.resume:
        if os.path.exists(args.checkpoint_path):
            ...
            print(f"--- Checkpoint loaded. Resuming from epoch {start_epoch}. Best F1: {best_val_f1:.4f} ---")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc=f"Epoch {epoch+1}/{args.epochs} [VAL]"):
                if batch is None: continue
                features, orange_labels, blue_labels = batch; features, orange_labels, blue_labels = features.to(device), orange_labels.to(device), blue_labels.to(device)
                orange_pred_prob, blue_pred_prob = model(features); total_val_loss += (criterion(orange_pred_prob, orange_labels) + criterion(blue_pred_prob, blue_labels)).item()
