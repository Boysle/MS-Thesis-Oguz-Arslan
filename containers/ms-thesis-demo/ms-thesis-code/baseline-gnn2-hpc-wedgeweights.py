...
class GraphLazyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # Extract necessary data for all players
            positions = x_tensor[:, 0:3]  # Assuming pos_x, pos_y, pos_z are the first 3 features
            velocities = x_tensor[:, 3:6] # Assuming vel_x, vel_y, vel_z are the next 3
            forwards = x_tensor[:, 6:9]   # Assuming forward_x, y, z are the next 3
            teams = x_tensor[:, 10]       # Assuming team is the 11th feature (index 10)

            edge_attrs = []
            # self.edge_index is shape [2, num_edges]. We iterate through each edge.
            for i, j in self.edge_index.t():
                # Feature 1: Inverse Distance
                dist = torch.linalg.norm(positions[i] - positions[j])
                d0, p = 1500.0, 2.0  # Your specified constants
                inv_dist = 1.0 / (1.0 + (dist / d0)**p)

                # Feature 2: Team Relationship
                same_team = 1.0 if teams[i] == teams[j] else 0.0

                # Vector from player i to player j
                vec_i_to_j = positions[j] - positions[i]

                # Feature 3: Forward Vector Alignment of player i towards j
                forward_align = normalized_dot_product(forwards[i], vec_i_to_j)

                # Feature 4: Velocity Vector Alignment of player i towards j
                # Scaled by normalized velocity magnitude
                vel_mag = torch.linalg.norm(velocities[i])
                max_vel = 2300.0 # From your normalization constants
                scaled_vel_mag = vel_mag / (max_vel + 1e-8)
                vel_align = normalized_dot_product(velocities[i], vec_i_to_j) * scaled_vel_mag

                edge_attrs.append([inv_dist, same_team, forward_align, vel_align])
    ...
    def empty_data(self):
        ...
        # NEW: Create a placeholder for edge_attr with the correct shape
        # self.edge_index.size(1) gets the number of edges (30)
        # 4 is the number of edge features we defined
