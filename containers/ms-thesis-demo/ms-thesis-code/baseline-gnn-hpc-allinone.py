...
def main():
    ...
    if not os.path.exists(best_model_path):
        print("--- No best model found. Skipping final evaluation. ---")
    else:
        ...
        model.eval(); all_test_oprobs, all_test_olabels, all_test_bprobs, all_test_blabels = [], [], [], []
        with torch.no_grad():
        ...
        print("\n--- Finding Optimal Thresholds using VALIDATION set probabilities ---")
        ...
        print(f"  Optimal Threshold for Orange: {optimal_threshold_orange:.4f}")
        print(f"  Optimal Threshold for Blue:   {optimal_threshold_blue:.4f}")

        print("\n--- Evaluating on TEST Set with OPTIMIZED Thresholds ---")
        test_preds_opt_o = (np.array(all_test_oprobs) > optimal_threshold_orange).astype(int)
        test_preds_opt_b = (np.array(all_test_bprobs) > optimal_threshold_blue).astype(int)
        final_f1_o = f1_score(all_test_olabels, test_preds_opt_o, zero_division=0); final_prec_o = precision_score(all_test_olabels, test_preds_opt_o, zero_division=0); final_recall_o = recall_score(all_test_olabels, test_preds_opt_o, zero_division=0)
        final_f1_b = f1_score(all_test_blabels, test_preds_opt_b, zero_division=0); final_prec_b = precision_score(all_test_blabels, test_preds_opt_b, zero_division=0); final_recall_b = recall_score(all_test_blabels, test_preds_opt_b, zero_division=0)

        print(f"  Optimized Orange -> F1: {final_f1_o:.4f} | Precision: {final_prec_o:.4f} | Recall: {final_recall_o:.4f}")
