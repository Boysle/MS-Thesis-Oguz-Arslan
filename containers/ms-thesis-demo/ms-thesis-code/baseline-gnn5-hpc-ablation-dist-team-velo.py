...
class GraphLazyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            for i, j in self.edge_index.t():
                ...
                # NEW: Feature 3: Velocity Vector Alignment
                vec_i_to_j = positions[j] - positions[i]
                vel_mag = torch.linalg.norm(velocities[i])
                max_vel = 2300.0 # Max velocity from your constants
                ...
                # Append a list with ALL THREE features
                edge_attrs.append([inv_dist, same_team, velocity_feature])

            # The shape will now be [num_edges, 3]
    ...
    def empty_data(self):
        ...
        edge_attr_tensor = torch.zeros((self.edge_index.size(1), 3), dtype=torch.float32)
...
def main():
    ...
    model = RocketLeagueGAT(PLAYER_FEATURES, GLOBAL_FEATURES, args.hidden_dim, edge_dim=3).to(device)
